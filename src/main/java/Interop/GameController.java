...
public class GameController {
	...
	private final double pheromoneDuration = 10; // SHOULD BE IN SCENARIO FILE
													// BUT ISN'T SO WE SET IT
													// MANUALLY HERE
	...
	public GameController(int gameMode, int height, int width, int numGuards,
			int numIntruders, double captureDistance,
			int winConditionIntruderRounds, double maxRotationAngle,
			double maxMoveDistanceIntruder, double maxSprintDistanceIntruder,
			double maxMoveDistanceGuard, int sprintCooldown,
			int pheromoneCooldown, double radiusPheromone,
			double slowDownModifierWindow, double slowDownModifierDoor,
			double slowDownModifierSentryTower, double viewAngle, int viewRays,
			double viewRangeIntruderNormal, double viewRangeIntruderShaded,
			double viewRangeGuardNormal, double viewRangeGuardShaded,
			double[] viewRangeSentry, double yellSoundRadius,
			double maxMoveSoundRadius, double windowSoundRadius,
			double doorSoundRadius, List<Rectangle> targetArea,
			List<Rectangle> spawnAreaIntruders, List<Rectangle> spawnAreaGuards,
			List<Rectangle> walls, List<Teleport> teleports,
			List<Rectangle> shaded, List<Rectangle> doors,
			List<Rectangle> windows, List<Rectangle> sentries) {
		// this.scenarioPercept = new ScenarioPercepts(gamemode, new
		// Distance(captureDistance), Angle.fromDegrees(maxRotationAngle), new
		// SlowDownModifiers(slowDownModifierWindow, slowDownModifierDoor,
		// slowDownModifierSentryTower), new Distance(radiusPheromone),
		// pheromoneCooldown);
		// this.scenarioIntruderPercepts = new
		// ScenarioIntruderPercepts(this.scenarioPercept,
		// winConditionIntruderRounds, new Distance(maxMoveDistanceIntruder),
		// new Distance(maxSprintDistanceIntruder), sprintCooldown);
		// this.scenarioGuardPercepts = new
		// ScenarioGuardPercepts(this.scenarioPercept, new
		// Distance(maxMoveDistanceGuard));
		...
		/*
		 * THIS IS FOR PERIOD 1 AS WE ONLY NEED AN EXPLORATION AGENT Later there
		 * needs to be a functionality, such that we can choose which groups
		 * intruder, and which groups guard to use in the game.
		 * 
		...
		if (numIntruders > 0) {
			...
			for (Intruder intruder : intruders) {
				double x = spawnAreaIntruders.get(0).getX()
						+ spawnAreaIntruders.get(0).getWidth()
								* random.nextDouble();
				double y = spawnAreaIntruders.get(0).getY()
						+ spawnAreaIntruders.get(0).getHeight()
								* random.nextDouble();
		...
		if (numGuards > 0) {
			...
			for (Guard guard : guards) {
				double x = spawnAreaGuards.get(0).getX()
						+ spawnAreaGuards.get(0).getWidth()
								* random.nextDouble();
				double y = spawnAreaGuards.get(0).getY()
						+ spawnAreaGuards.get(0).getHeight()
								* random.nextDouble();
			...
			switch (turn) {
				...
				case IntruderTurn :
					if (!intruders.isEmpty()) {
						for (Intruder intruder : intruders) {
							IntruderPercepts percept = intruderPercept(
									intruder);
	...
	private void guardAct(Action action, GuardPercepts percept, Guard guard) {
		if (action instanceof Move) {
			...
			Sound stepSound = new Sound(SoundPerceptType.Noise,
					new Distance(maxMoveSoundRadius));
			Ellipse2D stepSoundLocation = (Ellipse2D) guardLocations.get(guard)
					.clone();
			double newX = guardLocation.getX()
					+ Math.cos(guardDirection.getDegrees())
							* ((Move) action).getDistance().getValue();
			double newY = guardLocation.getY()
					+ Math.sin(guardDirection.getDegrees())
							* ((Move) action).getDistance().getValue();
			Ellipse2D newGuardLocation = new Ellipse2D.Double(newX, newY,
					guardLocation.getWidth(), guardLocation.getHeight());
			...
			Direction guardDirection = guardDirections.get(guard);
			double newGuardAngle = guardDirection.getDegrees()
					+ ((Rotate) action).getAngle().getDegrees();
			guardDirections.put(guard, Direction.fromDegrees(newGuardAngle));

		} else if (action instanceof Yell) {
			Sound yell = new Sound(SoundPerceptType.Yell,
					new Distance(yellSoundRadius));
			Ellipse2D yellLocation = new Ellipse2D.Double(
					guardLocations.get(guard).getX(),
					guardLocations.get(guard).getY(), yellSoundRadius * 2.0,
					yellSoundRadius * 2.0);
			soundLocations.put(yell, yellLocation);

		} else if (action instanceof DropPheromone) {
			if (guardPheromoneCooldowns.get(guard) == 0) {
				Smell smell = new Smell(((DropPheromone) action).getType(),
						new Distance(radiusPheromone));
				Ellipse2D smellLocation = new Ellipse2D.Double(
						guardLocations.get(guard).getX(),
						guardLocations.get(guard).getY(),
						smell.getRadius().getValue() * 2.0,
						smell.getRadius().getValue() * 2.0); // *2 = Radius ->
																// Diameter
	...
	private void intruderAct(Action action, IntruderPercepts percept,
			Intruder intruder) {
		if (action instanceof Move) {
			...
			Sound stepSound = new Sound(SoundPerceptType.Noise,
					new Distance(maxMoveSoundRadius));
			Ellipse2D stepSoundLocation = (Ellipse2D) intruderLocations
					.get(intruder).clone();
			double newX = intruderLocation.getX()
					+ Math.cos(intruderDirection.getDegrees())
							* ((Move) action).getDistance().getValue();
			double newY = intruderLocation.getY()
					+ Math.sin(intruderDirection.getDegrees())
							* ((Move) action).getDistance().getValue();
			Ellipse2D newintruderLocation = new Ellipse2D.Double(newX, newY,
					intruderLocation.getWidth(), intruderLocation.getHeight());
			...
			Direction intruderDirection = intruderDirections.get(intruder);
			double newintruderAngle = intruderDirection.getDegrees()
					+ ((Rotate) action).getAngle().getDegrees();
			intruderDirections.put(intruder,
					Direction.fromDegrees(newintruderAngle));
			...
			if (intruderSprintCooldowns.get(intruder) == 0) {
				...
				double newY = intruderLocation.getY()
						+ Math.sin(intruderDirection.getDegrees())
								* ((Move) action).getDistance().getValue();
				Ellipse2D newintruderLocation = new Ellipse2D.Double(newX, newY,
						intruderLocation.getWidth(),
						intruderLocation.getHeight());
				if (checkIfLegalMove(intruderLocation, newintruderLocation)) {
					intruderLocations.put(intruder, newintruderLocation);
					soundLocations.put(stepSound, stepSoundLocation);
				} else {
			...
			if (intruderPheromoneCooldowns.get(intruder) == 0) {
				Smell smell = new Smell(((DropPheromone) action).getType(),
						new Distance(radiusPheromone));
				Ellipse2D smellLocation = new Ellipse2D.Double(
						intruderLocations.get(intruder).getX(),
						intruderLocations.get(intruder).getY(),
						smell.getRadius().getValue() * 2.0,
						smell.getRadius().getValue() * 2.0);
	...
	private void smellDecay() {
		if (!intruderSmellLocations.isEmpty()) {
			for (Smell smell : intruderSmellLocations.keySet()) {
				...
				Distance newSmellRadius = new Distance(
						previousSmellRadius.getValue()
								- (radiusPheromone / pheromoneDuration));
				if (newSmellRadius.getValue() > 0) {
					Smell updatedSmell = new Smell(smell.getType(),
							newSmellRadius);
					Ellipse2D location = intruderSmellLocations.get(smell);
					Ellipse2D updatedLocation = new Ellipse2D.Double(
							location.getX(), location.getY(),
							newSmellRadius.getValue(),
							newSmellRadius.getValue());
